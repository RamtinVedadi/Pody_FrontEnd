...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async loadMoreChannels($state) {
                this.page += 1;
                if (this.$store.getters.userLogin === false) {
                    await this.$axios.post("http://localhost:8084/api/user/list/channels/" + this.page + "/" + this.size, {id: null}).then((response) => {
                        if (response.status === 200) {
                    ...
                    await this.$axios.post("http://localhost:8084/api/user/list/channels/" + this.page + "/" + this.size, {id: this.$store.state.user[0].userId}).then((response) => {
                        if (response.status === 200) {
                            const result = response.data;
                            if (result.length > 0) {
                                this.channels = this.channels.concat(result);
