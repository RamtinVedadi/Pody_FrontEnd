        ...
        <div class="pageMdAndUp hidden-md-only hidden-md-and-down hidden-sm-and-down"
            ...
            <div v-else>
                <v-row class="mr-2 pt-8 pr-1">
                    <h1 style="font-size: 28px">پادکست های من</h1>
                </v-row>
                <v-row>
                    <v-col cols="4">
                    ...
                    <v-row>
                        <v-col cols="6">
                            <v-row class="ma-4 mt-0 pt-0">
                                <h2 style="font-size: 20px">منتشر شده ها</h2>
                            </v-row>
                            <v-row no-gutters>
                        ...
                        <v-col cols="6">
                            <v-row class="ma-4 mt-0 pt-0">
                                <h2 style="font-size: 20px">منتشر نشده ها</h2>
                            </v-row>
                            <v-row no-gutters>
                                <v-flex v-for="item in unpublished" :key="item.podcastId" shrink>
                                    <NotPublishedCardPlayer :item="item" :title-count="18"/>
                                    <br/>
                                </v-flex>
                            </v-row>
                        </v-col>
...
<script>
    import NotPublishedCardPlayer from "../../components/cards/NotPublishedCardPlayer";
    ...
    export default {
        layout: "pody",
        name: "MyPodcasts",
        components: {NotPublishedCardPlayer, MobileCardPlayer, Loader, CardPlayer, InfiniteLoading},
        async asyncData(context) {
            return axios.get("http://localhost:8084/api/podcast/list/0/" + context.store.state.user[0].userId + "/user/" + 0 + "/" + 20).then(response => {
                return {
                    pageLoading: true,
                    podcasts: response.data.published,
        ...
        methods: {
            async loadMorePodcasts($state) {
                this.till += 1;
                await this.$axios.get("http://localhost:8084/api/podcast/list/0/" + this.$store.state.user[0].userId + "/user/" + this.till + "/" + this.to).then((response) => {
                    if (response.status === 200) {
                        const result = response.data.published;
                        if (result.length > 0) {
