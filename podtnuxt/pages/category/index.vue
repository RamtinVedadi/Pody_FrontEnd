      ...
      <div v-else>
        <v-row class="mr-2" v-for="(cat,index) in category" :key="index">
          <div v-if="cat.categoryPodcasts.length > 0">
            ...
            <div class="flexi mt-5" draggable="true">
            ...
            <script>
              ...
              export default {
                ...
                mounted() {
                  setTimeout(() => {
                    if (this.$store.getters.userLogin === false) {
                      axios.post("http://localhost:8084/api/category/list/page", {id: null}).then((response) => {
                        this.category = response.data;
                      });
                    } else {
                      axios.post("http://localhost:8084/api/category/list/page", {id: this.$store.state.user[0].userId}).then((response) => {
                        this.category = response.data;
                      });
                    }
                  }, 2000)
...
<script>
  ...
  export default {
    ...
    //   if (context.store.getters.userLogin === false) {
    //     await this.$axios.post("http://localhost:8084/api/category/list/page", {id: null}).then((response) => {
    //       this.category = response.data;
    //     });
    //   } else {
    //     await this.$axios.post("http://localhost:8084/api/category/list/page", {id: context.store.state.user[0].userId}).then((response) => {
    //       this.category = response.data;
    //     });
    //   }
    // },
