...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async publisherInfiniteLoading($state) {
                this.till += 1;
                await this.$axios.get("http://localhost:8084/api/podcast/list/1/" + this.$route.params.id + "/user/" + this.till + "/" + this.to).then((response) => {
                    this.isEnd = false;
                    if (response.status === 200) {
        ...
        mounted() {
            if (this.$route.params.id === this.$store.state.user[0].userId) {
